...
Class Wiki
{
	...
	private function preParse($data)
	{
		...
		//Pre Parse <phpw></phpw>
		$data = self::preParsePhpW($data);

		//Pre Parse <include>
		$data = preg_replace('"<include (.*?)>"ie', "self::preParseInclude(\"\\1\")", $data);
		...
		$data = preg_replace('"<files>"ie', "self::preParseFiles(\"\\1\")", $data);
		$data = preg_replace('"<files (.*?)>"ie', "self::preParseFiles(\"\\1\")", $data);
		...
		$data = self::preParsePhp($data);

		//Pre Parse <cmd>
		$data = self::preParseCmd($data);

		//Pre Parse <embed xxx yyy>
		$data = preg_replace('"<embed (.*?)>"ie', "self::preParseEmbed(\"\\1\")", $data);
	...
	private function preParseInclude($param)
	{
		...
		$post->content = self::preParsePhpW($post->content);
	...
	private function postParse($data)
	{
		...
		$data = preg_replace('"&lt;highlight(.*?)&gt;(.*?)&lt;/highlight&gt;"ie', "self::postParseHighlight(true, \"\\1\", \"\\2\")", $data); //Own Line
		$data = preg_replace('"&lt;highlight(.*?)&gt;(<br />|</p>)"ie', "self::postParseHighlight(false, \"\\1\")", $data); //Own Line
